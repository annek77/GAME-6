...
function makeLane(label, count, target) {
  ...
  for (let i = 0; i < slots; i++) {
    ...
    figuresHTML += `<div class="dr-slot" title="${isEmpty ? 'Empty' : (isOverflow ? 'No cabin left' : 'Taken')}">
      ${_cabinIcon(i + 1)}
  ...
  const balanceNote = overflow > 0
    ? `<span class="dr-overflow-note">+${overflow} without a cabin</span>`
    : missing > 0
      ? `<span class="dr-missing-note">${missing} cabin${missing > 1 ? 's' : ''} empty</span>`
...
function rReveal() {
  ...
  const womenLane = makeLane('Cabins for women', w, WOMEN_TARGET);
  const menLane   = makeLane('Cabins for men', m, MEN_TARGET);
  ...
  stage.innerHTML = `
    <div class="dr-reveal">
      ...
      <p class="dr-subtitle">
        ${totalSlots} cabins — ${WOMEN_TARGET} for women, ${MEN_TARGET} for men.
        Every seat was prepared.
      </p>
      ...
      <div class="dr-summary ${summaryClass}">
        ...
        ${!balanced ? `<span class="dr-imbalance">— ${Math.abs(w - m)} seat${Math.abs(w-m)!==1?'s':''} out of place</span>` : '<span class="dr-balanced">— balanced ✓</span>'}
      </div>

      <div class="dr-reflect">
        <p>
          The pool committee had planned the infrastructure for an even split —
          cabins, name tags, towels, everything <em>five a side</em>.
          Whoever didn't fit stood outside. Whoever was missing left empty seats.
        </p>
      </div>

      <button class="dr-btn-primary" onclick="go('outro')">So… what happened? →</button>
    </div>
    ...
    <style>
      ...
      .dr-summary strong { font-weight: 700; }
      .dr-summary--ok  { background: var(--ok,#4fb286); color: #fff; }
      .dr-summary--off { background: var(--pink,#ffc2df); }
      .dr-imbalance    { font-style: italic; opacity: .85; }
      .dr-balanced     { font-style: italic; }

      .dr-reflect {
        background: var(--paper, #fffdf6);
        border: 2px solid var(--ink,#143041);
        border-radius: 8px;
        padding: .9rem 1.1rem;
      ...
      .dr-reflect em { font-style: italic; }

      .dr-btn-primary {
        font-family: 'VT323', monospace;
        font-size: 1.5rem;
        background: var(--ink, #143041);
        color: var(--water, #a3e5f7);
        border: 3px solid var(--ink, #143041);
        border-radius: 6px;
      ...
      .dr-btn-primary:hover {
        background: var(--water-deep, #5cb9da);
        color: var(--ink, #143041);
      }
    </style>
  `;
...
/* ----------------------------------------------------------
   rIntermezzo()
   Terminal-Ticker sequence before the pool reveal.
   Lines appear one per second, then "Enter the pool →" button.
   ---------------------------------------------------------- */
function rIntermezzo() {
  const { w, m } = countInvited();
  const delay     = state.delayWeeks || 0;
  const ext       = state.extensions  || 0;

  const lines = [
    'Jury complete.',
    'Invitations sent.',
    delay > 0
      ? `+${delay} week${delay !== 1 ? 's' : ''} delay, ${ext} extension${ext !== 1 ? 's' : ''}.`
  ...
  stage.innerHTML = `
    <div class="im-terminal" id="im-terminal">
      <div class="im-lines" id="im-lines"></div>
      <div class="im-cursor" id="im-cursor">█</div>
      <div class="im-cta" id="im-cta" style="display:none">
        <button class="im-btn" onclick="go('reveal')">Enter the pool →</button>
      </div>
    </div>

    <style>
      .im-terminal {
        /* 100vh made the stage scroll oddly inside the frame — 60vh fills the view without overflowing */
        min-height: 60vh;
        background: var(--ink, #143041);
        color: #e8f8ff;
        font-family: 'VT323', monospace;
      ...
      .im-cursor {
        color: var(--water, #a3e5f7);
        animation: im-blink .8s step-end infinite;
        font-size: 1.2em;
        line-height: 1;
        margin-top: .1rem;
      }
      ...
      .im-btn {
        ...
        background: var(--water, #a3e5f7);
        color: var(--ink, #143041);
        border: 3px solid var(--water, #a3e5f7);
        border-radius: 6px;
      ...
      .im-btn:hover {
        background: var(--water-deep, #5cb9da);
        border-color: var(--water-deep, #5cb9da);
        color: #fff;
      }
    </style>
  `;

  // Animate lines one per second
  const container = document.getElementById('im-lines');
  const cursor    = document.getElementById('im-cursor');
  const cta       = document.getElementById('im-cta');
  ...
  lines.forEach((text, i) => {
    ...
    setTimeout(() => {
      ...
      // After last line: hide cursor, show button
      if (i === lines.length - 1) {
        setTimeout(() => {
