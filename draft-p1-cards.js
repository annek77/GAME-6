...
function _drawP1Card() {
  ...
  shell.appendChild(el(`<div class="p1-progress">
    Candidate ${idx + 1} of ${total} &nbsp;·&nbsp; ${chosen}/${JURY_SIZE} chosen
  </div>`));
  ...
  if (chosen >= JURY_SIZE && !isSelected) {
    ...
    const full = el(`<button class="btn-add" disabled style="opacity:.45;cursor:not-allowed">
      Jury already full (${JURY_SIZE}/${JURY_SIZE})
    </button>`);
    actions.appendChild(full);
  } else if (isSelected) {
    // Already in jury — allow removing (stay on this card so the change is visible)
    ...
    const add = el(`<button class="btn-add">Add to jury</button>`);
    add.onclick = () => {
  ...
  const skip = el(`<button class="btn-skip">Skip</button>`);
  skip.onclick = () => {
    state.p1Idx++;
    _drawP1Card();
  };
  ...
  if (idx > 0) {
    const back = el(`<button class="btn-back-card">← Previous card</button>`);
    back.onclick = () => {
      state.p1Idx--;
      _drawP1Card();
    };
...
function _drawP1Done() {
  ...
  if (chosen >= JURY_SIZE) {
    ...
    shell.appendChild(el(`<div class="p1-done">
      <h2>You've seen every candidate</h2>
      <p>${chosen}/${JURY_SIZE} chosen. You can confirm the jury now.</p>
    </div>`));
    ...
    shell.appendChild(el(`<div class="p1-done">
      <h2>You've seen every candidate</h2>
      <p>You've filled only ${chosen} of ${JURY_SIZE} seats.
         Go back and pick ${JURY_SIZE - chosen} more.</p>
    </div>`));
    const restart = el(`<button class="btn ghost" style="font-size:16px">
      Browse the pool again
    </button>`);
    restart.onclick = () => {
      state.p1Idx = 0;
      _drawP1Card();
...
function _makeP1Card(p) {
  ...
  CRITERIA.forEach(cr => {
    const chip = el(`<span class="chip shown" data-k="${cr.key}">
      ${cr.label} <span class="val">${"●".repeat(p.sig[cr.key]) || "–"}</span>
    </span>`);
    crit.appendChild(chip);
  });

  const comp = el(`<span class="chip comp shown">
    ${COMPETENCE.label} <span class="val">${"●".repeat(p.sig.publicValue) || "–"}</span>
  </span>`);
...
/* Keep makeCard as the alias used elsewhere (phase-2 cards etc. are separate) */
function makeCard(p) {
  return _makeP1Card(p);
}

/* -----------------------------------------------------------------------
   Unchanged helpers — kept here so the draft is self-contained
   (If already in game.js, skip these — or ensure they match.)
   ----------------------------------------------------------------------- */
...
/* ==========================================================================
   ...
   1. state initialisation (top of file, inside the `const state = { … }` literal):
      Add:   p1Idx: 0,
      Remove weights / peekedCompetence if you no longer need them,
             or just leave them — they're harmless.

   2. resetGame() — add one line:
        state.p1Idx = 0;
      and you can remove:
        state.weights = { availability:0, assertiveness:0, prestige:0, seniority:0 };
        state.peekedCompetence = 0;
      (safe to keep them too — they just won't be used)

   3. Delete or comment out: peek(), peekComp() — no longer needed.

   4. CSS: paste the CSS block from the top of this file into your stylesheet
      (or a <style> in index.html).  The .p1-* classes are new; existing
      .chip.shown styles continue to work unchanged.

   5. The grid-based renderHandPick() in game.js is fully replaced by the
      version above.  The rest of game.js (rPhase1, rHandoff, rApps, …)
      is unchanged.
   ========================================================================== */
